...
export default function SecondaryButton({ children, onClick, icon, color = "red", className }) {
  return (
    <button
      className={`flex cursor-pointer items-center gap-2 rounded-sm border ${color === "red" ? "border-[var(--my-color-red)]" : "border-[var(--my-color-blue)]"} px-3 py-1 ${color === "red" ? "text-[var(--my-color-red)]" : "text-[var(--my-color-blue)]"}`}
      onClick={onClick}
    >
